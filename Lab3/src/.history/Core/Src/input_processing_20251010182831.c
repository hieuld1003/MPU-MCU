...
// Thêm biến để kiểm soát tốc độ tăng giá trị
...
void fsm_for_input_processing(void){
    ...
    if (is_button_pressed(1)) {
        if (!button1_flag) {
            ...
            // Chỉ tăng giá trị khi ở mode chỉnh sửa và khi mới nhấn nút
            if (mode != NORMAL_MODE) {
                temp_value++;
        ...
        if (is_button_pressed_1s(1)) {
            if (!long_press_flag) {
                long_press_flag = 1; // Đánh dấu là đã bắt đầu tự động tăng
                long_press_counter = 0; // Reset bộ đếm
            }
            
            // Đếm số chu kỳ
            long_press_counter++;
            
            // Chỉ tăng giá trị sau mỗi 100 chu kỳ (1 giây)
            if (long_press_counter >= 10000) {
        ...
        button1_flag = 0;
        long_press_flag = 0; // Reset flag khi thả nút
        long_press_counter = 0; // Reset bộ đếm khi thả nút
    }
