...
int timer_blinking_counter = 0; // Thêm biến đếm
int timer_blinking_flag = 0;    // Thêm cờ
...
void setTimer_Blinking(int duration){ // Thêm hàm set
    timer_blinking_counter = duration / 10;
...
void timer_run(){
    ...
    if(timer_blinking_counter > 0){ // Thêm logic đếm
        timer_blinking_counter--;
        if(timer_blinking_counter <= 0){
