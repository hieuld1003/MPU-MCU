...
static int mode = 0; // Đổi giá trị ban đầu từ NORMAL_MODE thành 0
...
void fsm_for_input_processing(void){
    ...
    if (!is_initialized) {
        ...
        mode = NORMAL_MODE; // Đặt rõ ràng là mode 1 khi khởi tạo
        ...
        setTimer_7seg_scan(10);     // Bắt đầu quét LED 7-seg
    ...
    // Xử lý nút nhấn sw0 để chuyển mode - SỬA LẠI
    if (is_button_pressed(0)) {
        if (button0_flag == 0) { // Chỉ thực hiện một lần khi nút được nhấn
            button0_flag = 1;
            mode = (mode % 4) + 1; // Chuyển đổi giữa mode 1-4
        }
    } else {
        button0_flag = 0; // Đặt lại cờ khi nút được thả ra
    }
    
    // Xử lý các mode khác nhau
    switch(mode) {
        case NORMAL_MODE:
            // Tắt tất cả đèn khi chuyển từ mode khác về mode 1
            // và khôi phục trạng thái đèn hiện tại
            ...
            switch(status_h) {
                case H_RED:
                    ...
                    break;
                case H_GREEN:
                    set_red_h(0); set_yellow_h(0); set_green_h(1);
                    if (counter_h <= 0) {
                        status_h = H_YELLOW;
                    ...
                    break;
                case H_YELLOW:
                    set_red_h(0); set_yellow_h(1); set_green_h(0);
                    if (counter_h <= 0) {
                        status_h = H_RED;
                        counter_h = time_red;
            ...
            switch(status_v) {
                case V_RED:
                    set_red_v(1); set_yellow_v(0); set_green_v(0);
                    if (counter_v <= 0) {
                        status_v = V_GREEN;
                        counter_v = time_green;
                ...
                case V_GREEN:
                    ...
                    break;
                case V_YELLOW:
                    set_red_v(0); set_yellow_v(1); set_green_v(0);
                    if (counter_v <= 0) {
                        status_v = V_RED;
                        counter_v = time_red;
