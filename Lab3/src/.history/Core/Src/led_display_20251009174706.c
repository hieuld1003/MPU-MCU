...
void scan7SEG() {
    if (timer_7seg_scan_flag == 1) {
        setTimer_7seg_scan(500); // Quét nhanh giữa 2 giai đoạn
...
void set_red_h(int state) {
    if (state == -1) HAL_GPIO_TogglePin(red_h_GPIO_Port, red_h_Pin);
    else HAL_GPIO_WritePin(red_h_GPIO_Port, red_h_Pin, !state);
}
void set_yellow_h(int state) {
    if (state == -1) HAL_GPIO_TogglePin(yellow_h_GPIO_Port, yellow_h_Pin);
    else HAL_GPIO_WritePin(yellow_h_GPIO_Port, yellow_h_Pin, !state);
}
void set_green_h(int state) {
    if (state == -1) HAL_GPIO_TogglePin(green_h_GPIO_Port, green_h_Pin);
    else HAL_GPIO_WritePin(green_h_GPIO_Port, green_h_Pin, !state);
}

void set_red_v(int state) {
    if (state == -1) HAL_GPIO_TogglePin(red_v_GPIO_Port, red_v_Pin);
    else HAL_GPIO_WritePin(red_v_GPIO_Port, red_v_Pin, !state);
}
void set_yellow_v(int state) {
    if (state == -1) HAL_GPIO_TogglePin(yellow_v_GPIO_Port, yellow_v_Pin);
    else HAL_GPIO_WritePin(yellow_v_GPIO_Port, yellow_v_Pin, !state);
}
void set_green_v(int state) {
    if (state == -1) HAL_GPIO_TogglePin(green_v_GPIO_Port, green_v_Pin);
    else HAL_GPIO_WritePin(green_v_GPIO_Port, green_v_Pin, !state);
}