...
static int mode = NORMAL_MODE; // Thêm lại biến mode
...
void fsm_for_input_processing(void){
    ...
    GPIO_PinState sw0_state = HAL_GPIO_ReadPin(sw0_GPIO_Port, sw0_Pin);
    
    // Chuyển chế độ ngay khi phát hiện nút được nhấn
    // Đổi GPIO_PIN_SET thành GPIO_PIN_RESET nếu logic vẫn bị ngược
    if(sw0_state == GPIO_PIN_SET) {
        // Debug: Hiển thị "8888" trên tất cả các đèn LED khi nút được nhấn
        led_buffer[0] = 8;
        led_buffer[1] = 8;
        led_buffer[2] = 8;
        led_buffer[3] = 8;
        ...
        // Chuyển mode
        mode = MODIFY_RED_MODE; // Chỉ chuyển đến mode cố định để debug
    }
