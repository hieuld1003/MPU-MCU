...
void fsm_for_input_processing(void) {
            ...
            if(is_button_pressed(0)) {
                ...
                switch(currentMode) {
                    ...
                    case MODE_GREEN:
                        ...
                        break;
                    case MODE_NORMAL:
                        trafficLightInit(); // Reinitialize traffic lights
                        break;
                }
                
                // Reset auto-increment timer
                timerForAutoInc = 0;
            }
            break;
    ...
    switch(currentMode) {
        case MODE_NORMAL:
            ...
            updateDisplays(MODE_NORMAL, 0);
            break;
            
        case MODE_RED:
            clearBlinkingLeds();
            blinkRedLeds();
            updateDisplays(MODE_RED, tempValue);
            break;
            
        case MODE_AMBER:
            clearBlinkingLeds();
            blinkAmberLeds();
            updateDisplays(MODE_AMBER, tempValue);
            break;
            
        case MODE_GREEN:
            clearBlinkingLeds();
            blinkGreenLeds();
            updateDisplays(MODE_GREEN, tempValue);
            break;
    }
}
