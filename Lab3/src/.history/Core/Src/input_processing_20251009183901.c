...
static int button0_pressed = 0;
static int button0_released = 0;
...
void fsm_for_input_processing(void){
    ...
    // Xử lý nút nhấn theo kiểu rising edge detection
    if (is_button_pressed(0)) {
        if (!button0_pressed) {
            button0_pressed = 1;
            
            // Chuyển mode khi phát hiện nút vừa được nhấn
            mode = (mode % 4) + 1;
        }
    } else {
        button0_pressed = 0;
    ...
    // Hiển thị trạng thái nút nhấn ở vị trí cuối cùng (LED debugging)
    // Chỉ sử dụng khi debug, có thể bỏ comment dòng này
    // led_buffer[3] = button0_pressed;
