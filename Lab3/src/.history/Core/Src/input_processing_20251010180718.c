...
// Biến cho chức năng tự động tăng giá trị
...
void fsm_for_input_processing(void){
    ...
    if (mode != NORMAL_MODE) {  // Chỉ xử lý trong các mode chỉnh sửa
        if (is_button_pressed(1)) {
            if (!button1_flag) {
                button1_flag = 1;
                // Tăng giá trị ngay khi nhấn nút
                temp_value++;
                if (temp_value > 99) temp_value = 1;
            }
            
            // Xử lý khi giữ nút lâu hơn 1 giây
            if (is_button_pressed_1s(1)) {
                auto_increase_counter++;
                if (auto_increase_counter >= 20) {  // Mỗi 200ms tăng một lần (20 * 10ms)
                    temp_value++;
                    if (temp_value > 99) temp_value = 1;
