...
void blinkRedLeds(void) {
    ...
    if (timerFlag(&timerForBlink)) {
        ...
        HAL_GPIO_WritePin(red_h_GPIO_Port, red_h_Pin, blinkState ? 1 : 0);
        HAL_GPIO_WritePin(red_v_GPIO_Port, red_v_Pin, blinkState ? 1 : 0);
...
void blinkAmberLeds(void) {
    ...
    if (timerFlag(&timerForBlink)) {
        blinkState = !blinkState;
        HAL_GPIO_WritePin(yellow_h_GPIO_Port, yellow_h_Pin, blinkState ? 1 : 0);
        HAL_GPIO_WritePin(yellow_v_GPIO_Port, yellow_v_Pin, blinkState ? 1 : 0);
        setTimer(&timerForBlink, 500); // 0.5s (2Hz)
    }
}
...
void blinkGreenLeds(void) {
    ...
    if (timerFlag(&timerForBlink)) {
        blinkState = !blinkState;
        HAL_GPIO_WritePin(green_h_GPIO_Port, green_h_Pin, blinkState ? 1 : 0);
        HAL_GPIO_WritePin(green_v_GPIO_Port, green_v_Pin, blinkState ? 1 : 0);
        setTimer(&timerForBlink, 500); // 0.5s (2Hz)
    }
}
...
void updateDisplays(void) {
    ...
    if (currentMode == MODE_NORMAL) {
        ...
        // Configuration mode (2, 3, or 4)
        if (displayCount < 2) {
            // First two displays show the value being configured
            int digit = displayCount == 0 ? tempValue / 10 : tempValue % 10;
            display7SEG_h(digit);
        } else {
            // Last two displays show the mode number (fixed, not alternating)
            int digit = displayCount == 2 ? 0 : currentMode;
            display7SEG_v(digit);
        }
    }
}