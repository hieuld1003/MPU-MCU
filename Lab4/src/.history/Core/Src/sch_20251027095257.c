...
void SCH_Update(void) {
    ...
    if (head_index != NO_TASK) {
        if (current_time_ticks >= SCH_tasks_G[head_index].Next_Run_Time) {
            SCH_tasks_G[head_index].RunMe += 1;
            
            if (SCH_tasks_G[head_index].Period > 0) {
                SCH_tasks_G[head_index].Next_Run_Time = current_time_ticks + SCH_tasks_G[head_index].Period;
...
void SCH_Dispatch_Tasks(void) {
    uint8_t current = head_index;
    uint8_t prev = NO_TASK;
    
    while (current != NO_TASK) {
        if (SCH_tasks_G[current].pTask && SCH_tasks_G[current].RunMe > 0) {
            uint8_t run_count = SCH_tasks_G[current].RunMe;
            SCH_tasks_G[current].RunMe = 0;
            
            while (run_count > 0) {
                (*SCH_tasks_G[current].pTask)();
                run_count--;
            }
            
            if (SCH_tasks_G[current].Period == 0) {
                uint8_t next = SCH_tasks_G[current].Next_Index;
                SCH_Delete_Task(SCH_tasks_G[current].TaskID);
                current = next;
                continue;
            }
        }
