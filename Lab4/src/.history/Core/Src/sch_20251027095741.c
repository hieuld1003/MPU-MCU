...
void SCH_Update(void) {
    ...
    // Xử lý TẤT CẢ tasks có Next_Run_Time <= current_time_ticks
    while (head_index != NO_TASK && 
        ...
        if (SCH_tasks_G[head_index].Period > 0) {
            // Periodic task: tính thời điểm chạy tiếp theo
            ...
            // Insert lại vào sorted list
            SCH_Insert_Task_Sorted(current_task_index);
        } else {
            // One-shot task: chỉ xóa khỏi đầu list
            head_index = SCH_tasks_G[head_index].Next_Index;
        }
    }
}
